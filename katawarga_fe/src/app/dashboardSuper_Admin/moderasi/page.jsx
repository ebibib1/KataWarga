...
export default function SuperAdminModeration() {
  ...
  const fetchFlags = async () => {
    ...
    try {
      ...
      console.error("Error fetching flags in super admin:", err);
      setFlags([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdateFlag = async (flagId, action) => {
    const confirmMsg =
      action === "delete_report"
        ? "Apakah Anda yakin ingin menghapus laporan aduan yang dilaporkan secara permanen? Tindakan ini akan menghapusnya dari beranda warga."
        : "Apakah Anda yakin ingin mengabaikan flag ini? Laporan aduan warga akan tetap aman dan ditampilkan.";
    ...
    try {
      const res = await fetch(`${apiUrl}/flags/${flagId}`, {
        ...
        body: JSON.stringify({
          status: "reviewed",
          action: action // dismiss OR delete_report
        }),
      });
      ...
      if (res.ok) {
        alert(
          action === "delete_report"
            ? "Tindakan diselesaikan: Laporan telah dihapus secara fisik."
            : "Tindakan diselesaikan: Flag diabaikan, laporan aman."
        );
        fetchFlags();
      } else {
      ...
      alert("Mode Offline: Aksi flag berhasil diselesaikan.");
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold font-display text-[#111827]">
          Otoritas & Moderasi Flag
        </h1>
        <p className="text-sm text-[#6B6B8A]">
          Hak akses penuh Super Admin: Tinjau aduan bermasalah, abaikan flag, atau hapus laporan aduan warga yang melanggar.
        </p>
      </div>
      ...
      <div className="bg-white p-3.5 rounded-2xl border border-[#E8E2D9] shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 text-[10px] font-bold text-red-700 bg-red-50 border border-red-100 px-3 py-2 rounded-xl">
          <ShieldAlert size={14} className="shrink-0" />
          <span>Super Admin berhak menghapus laporan secara fisik dengan mengeklik tombol silang merah (X).</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#E8E2D9] shadow-sm overflow-hidden">
        {loading ? (
                      ...
                      {flag.status === "pending" ? (
                        <div className="flex justify-end gap-1.5">
                          ...
                          <button
                            ...
                            title="Abaikan Flag (Laporan Aman)"
                          >
                          ...
                          <button
                            ...
                            title="Hapus Laporan Aduan Warga dari Database"
                          >
                            {processingId === flag.id ? (
                              <Loader2 className="w-3.5 h-3.5 animate-spin" />
                            ) : (
          ...
          <div className="py-20 text-center flex flex-col items-center justify-center gap-2">
            ...
            <h3 className="font-semibold text-[#111827]">Tidak ada antrean flag</h3>
            <p className="text-xs text-[#6B6B8A] max-w-xs">
              Antrean moderasi kosong. Seluruh aduan warga aman dari indikasi pelanggaran ketentuan.
            </p>
          </div>
        )}
      </div>
