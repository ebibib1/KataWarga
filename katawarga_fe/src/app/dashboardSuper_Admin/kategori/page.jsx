...
export default function SuperAdminCategories() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setCategoryName("");
      setCategoryIcon("tag");
      alert("Mode Offline: Kategori berhasil diperbarui.");
    } finally {
  ...
  const handleDeleteCategory = async (catId) => {
    if (!confirm("Apakah Anda yakin ingin menghapus kategori aduan warga ini? Laporan aduan yang terasosiasi dengan kategori ini mungkin akan terpengaruh.")) return;

    const token = session.accessToken || session.user?.accessToken;

    try {
      const res = await fetch(`${apiUrl}/categories/${catId}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      alert("Mode Offline: Kategori berhasil dihapus.");
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl font-bold font-display text-[#111827]">
          Otoritas & Manajemen Kategori
        </h1>
        <p className="text-sm text-[#6B6B8A]">
          Hak akses penuh Super Admin: Daftarkan jenis masalah baru, perbarui kode penanda, dan hapus kategori global.
        </p>
      </div>
                ...
                >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-rose-50 text-[#6B6B8A] hover:text-rose-600 transition"
        ...
        <div className="bg-white p-5 rounded-2xl border border-[#E8E2D9] shadow-sm h-fit">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input
                type="text"
                placeholder="Contoh: wind, shield, trash, road"
                value={categoryIcon}
