...
const NAV_ITEMS = [
  { href: "/dashboardSuper_Admin",           icon: LayoutDashboard, label: "Ringkasan Analitis" },
  { href: "/dashboardSuper_Admin/pengguna",  icon: Users,           label: "Manajemen Pengguna" },
  { href: "/dashboardSuper_Admin/admin",     icon: Shield,          label: "Manajemen Admin" },
  { href: "/dashboardSuper_Admin/laporan",   icon: FileText,        label: "Manajemen Laporan" },
  { href: "/dashboardSuper_Admin/moderasi",  icon: Flag,            label: "Moderasi Flag" },
  { href: "/dashboardSuper_Admin/kategori",  icon: Layers,          label: "Kategori" },
  { href: "/dashboardSuper_Admin/pengaturan",icon: Settings,        label: "Pengaturan" },
];
...
export default function SuperAdminLayout({ children }) {
    ...
    return (
      <div className="min-h-screen bg-[#FFFBF5] flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#3D3D5C] text-sm font-medium">Memverifikasi otorisasi super admin...</p>
        </div>
      </div>
    );
  }
  ...
  const isActive = (href) => {
    if (href === "/dashboardSuper_Admin") return pathname === "/dashboardSuper_Admin";
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-[#E8E2D9] bg-white h-screen sticky top-0 flex-shrink-0">
        ...
        <div className="h-16 flex items-center gap-2.5 px-6 border-b border-[#E8E2D9]">
          ...
          <div>
            ...
            <span className="block text-[10px] text-red-600 font-bold tracking-wider uppercase">
              Super Admin Panel
            </span>
          </div>
        </div>
        ...
        <header className="h-16 border-b border-[#E8E2D9] bg-white px-4 md:px-6 flex items-center justify-between sticky top-0 z-40">
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex flex-col">
              <span className="text-xs text-[#6B6B8A]">Sistem Administrator,</span>
              <span className="text-sm font-semibold text-red-600 flex items-center gap-1">
                {session.user.name}
                <span className="text-[9px] bg-red-100 text-red-800 font-bold px-1.5 py-0.2 rounded uppercase tracking-wider">ROOT</span>
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/dashboardSuper_Admin/pengaturan" className="flex items-center gap-2">
              <Avatar
                src={session.user.image}
                initials={userInitials}
                color="bg-red-600"
